...
def check_neck(request):
    ...
    if serializer.is_valid():
        ...
        # base 64 => Image
        image = base64.b64decode(image_blob)
        print(image)
        # Image File => numpy.ndarray[(rows,cols,3),uint8]
        nd_image = np.fromstring(image,  sep='\t')
        print(nd_image)

        RGB_image = cv2.cvtColor(nd_image,cv2.COLOR_BGR2RGB)

        landmark_list = face_landmark.get_landmark(RGB_image)
        print(landmark_list)
